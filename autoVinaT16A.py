...
def getCoord(cid):
    pcp.download("SDF", "linux_%s/%s.sdf" %
                 (cid, cid), cid, "cid", overwrite=True)
    return
...
def SDFtoPDBQT(cid):
    os.system("obabel linux_%s/%s.sdf -O linux_%s/%s.pdbqt --gen3d" %
              (cid, cid, cid, cid))
    return
...
def linuxInput(cid):
    ...
    for K in range(1, run + 1):
        ...
        f = open("C:/Users/jwroh/Desktop/VM folder/%s_linuxRun_%d.sh" %
                 (cid, K), "w")
        f.write("#!/bin/tcsh\n")
        ...
        f.write(
            "    vina --config %s_conf.txt --log logResult/%s_log_%d_${cnt}.txt --out outResult/%s_out_%d_${cnt}.pdbqt\n" % (cid, cid, K, cid, K))
        f.write("    @ cnt += 1\n")
        f.write("end\n")
        f.close()
...
def cenMass(cid):
    ...
    for file in os.listdir("linux_%s/outResult" % (cid)):
        ...
        structure = parser.get_structure(
            "LIG", "linux_%s/outResult/%s" % (cid, file))
...
def cenMassSym(cid):
    ...
    for line in f:
        ...
        if min(disToCenA, disToCenB) == disToCenA:
            ...
            X1 = (X - cenX) * np.cos(np.deg2rad(180)) - \
                (Y - cenY) * np.sin(np.deg2rad(180)) + cenX
            Y1 = (X - cenX) * np.sin(np.deg2rad(180)) + \
                (Y - cenY) * np.cos(np.deg2rad(180)) + cenY
            g.write("%.3f\t%.3f\t%.3f\t\n" % (X1, Y1, Z))
...
def KMEANScluster(cid):
    ...
    while True:
        ...
        for i in range(cluster):
            ...
            mean.append(
                [len(tempX), [np.mean(tempX), np.mean(tempY), np.mean(tempZ)]])
...
def resultOutput(cid):
    ...
    os.system(
        "obabel analysis_%s/pocketDocking_out_*.pdbqt -O analysis_%s/pocketDocking__final.pdb" % (cid, cid))
    return
